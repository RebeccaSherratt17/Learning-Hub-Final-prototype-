...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (!ids || ids.length === 0) {
      return NextResponse.json({ error: 'No learning path IDs provided' }, { status: 400 })
    }
    ...
    console.error('Failed to bulk update learning paths:', error)
    return NextResponse.json({ error: 'Failed to bulk update learning paths' }, { status: 500 })
  }
}
