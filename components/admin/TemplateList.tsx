...
import { checkTemplateHealth } from '@/lib/admin/metadataHealth'
...
export default function TemplateList() {
  ...
  const [bulkTier, setBulkTier] = useState<AccessTier>('GATED')
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      const url = `/api/admin/templates/${deleteConfirm.id}${useConfirmParam ? '?confirm=true' : ''}`
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={8} className="px-4 py-12 text-center">
                  <span className="material-symbols-sharp text-[40px] text-diligent-gray-3">description</span>
              ...
              sortedTemplates.map((template) => (
                <tr key={template.id} className="border-b border-diligent-gray-1 hover:bg-diligent-gray-1/50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      {(() => {
                        const missing = checkTemplateHealth(template)
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${tierStyles[template.accessTier]}`}>
                      {tierLabels[template.accessTier]}
                    </span>
                  </td>
        ...
        <ConfirmModal
          ...
          message={
            deleteConfirm.affectedPaths
              ? `This template is referenced by ${deleteConfirm.affectedPaths.length} learning path(s): ${deleteConfirm.affectedPaths.map((p) => p.title).join(', ')}. Are you sure you want to delete "${deleteConfirm.title}"?`
